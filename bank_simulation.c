...
void vTeller(void *pvParameters)
{
	...
	for(;;)
	{
		if(hqueue != 0)
		{
			...
			{
				if(id == 2){
					HAL_UART_Transmit(&huart2, (uint8_t *)"\t", 1, UART_TIMEOUT);
					n = sprintf((char *)buffer, "\tTeller %d ", id);
					HAL_UART_Transmit(&huart2, buffer, n, UART_TIMEOUT);
					n = sprintf((char *)buffer, "busy for %d ticks \n\r", customer_tt);
					HAL_UART_Transmit(&huart2, buffer, n, UART_TIMEOUT);
				}
				else if(id == 3)
				{
					HAL_UART_Transmit(&huart2, (uint8_t *)"\t\t", 2, UART_TIMEOUT);
					n = sprintf((char *)buffer, "\t\tTeller %d ", id);
					HAL_UART_Transmit(&huart2, buffer, n, UART_TIMEOUT);
					n = sprintf((char *)buffer, "busy for %d ticks \n\r", customer_tt);
					HAL_UART_Transmit(&huart2, buffer, n, UART_TIMEOUT);
